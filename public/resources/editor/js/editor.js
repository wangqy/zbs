 ...
 */
function getText(){
    var tmpSCurrMode = sCurrMode;
    sCurrMode = "TEXT";
    var content = getHTML();
    sCurrMode = tmpSCurrMode;
    return content;
}

/**
 * 校验editor的内容是否为空，即校验纯文本内容是否只包含全角空格
 * @author MaHB
 * @date 2006.4.25
 */
function isEmpty(){
    var whiteSpace = String.fromCharCode("12288")   //全角空格
                   + String.fromCharCode("32")      //英文空格
                   + String.fromCharCode("9")       //Tab空格，即"\t"
                   + String.fromCharCode("13")      //回车，即"\r"
                   + String.fromCharCode("10");     //换行，即"\n"
    var emptyReg = new RegExp("^[" + whiteSpace + "]+$");
    return emptyReg.test(getText());
}
